...
MapReportController.$inject = ['$http', '$scope', '$rootScope', 'Notify', 'PostLockService', '$state', 'LoadingProgress', 'Util'];

function MapReportController($http, $scope, $rootScope, Notify, PostLockService, $state, LoadingProgress, Util) {
    ...
    $http({
        ...
        var data1 = Object.keys(cat).map(function (k) {
            return cat[k]
        });
        cat = response.data.events.total_by_day.violencia_armada;
        var data2 = Object.keys(cat).map(function (k) {
            return cat[k]
        });
